...
# 3. Чистая установка Python
RUN python3 -m pip install --upgrade pip

# Устанавливаем зависимости FaceFusion, НО УДАЛЯЕМ из списка onnxruntime
RUN sed -i '/onnxruntime/d' requirements.txt
RUN python3 -m pip install -r requirements.txt
RUN python3 -m pip install runpod requests gdown

# КРУТО: Ставим именно GPU версию в последнюю очередь
RUN python3 -m pip uninstall -y onnxruntime onnxruntime-gpu
RUN python3 -m pip install onnxruntime-gpu==1.18.0 --extra-index-url https://aiinfra.pkgs.visualstudio.com/PublicPackages/_apis/packaging/Feeding/onnxruntime-cuda-12/nuget/index
...
# Переменные окружения для CUDA 12.1
